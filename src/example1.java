...
public class example1 {
	...
	public static PGPPublicKey readPublicKey(InputStream in) throws IOException, PGPException {
		...
		PGPPublicKeyRing keyRing = new PGPPublicKeyRing(toByteArray(in), new JcaKeyFingerprintCalculator());
		ArrayList<PGPPublicKeyRing> keyList = new ArrayList<PGPPublicKeyRing>();
		keyList.add(keyRing);
		PGPPublicKeyRingCollection pgpPub = new PGPPublicKeyRingCollection(keyList);
	...
	public static void decryptFile(InputStream in, OutputStream out, InputStream keyIn, char[] passwd)
		 ...
		 * in = org.bouncycastle.openpgp.PGPUtil.getDecoderStream(in);
		 * 
		 * PGPObjectFactory pgpF = new PGPObjectFactory(toByteArray(in), new
		 * JcaKeyFingerprintCalculator()); PGPEncryptedDataList enc;
		 * 
		 * Object o = pgpF.nextObject(); // // the first object might be a PGP marker
		 * packet. // if (o instanceof PGPEncryptedDataList) { enc =
		 * (PGPEncryptedDataList) o; } else { enc = (PGPEncryptedDataList)
		 * pgpF.nextObject(); }
		 * 
		 * // // find the secret key // Iterator<PGPPublicKeyEncryptedData> it = enc.;
		 * PGPPrivateKey sKey = null; PGPPublicKeyEncryptedData pbe = null;
		 * 
		 * while (sKey == null && it.hasNext()) { pbe = it.next();
		 * 
		 ...
		 * 
		 * if (message instanceof PGPCompressedData) { PGPCompressedData cData =
		 * (PGPCompressedData) message; PGPObjectFactory pgpFact = new
		 * PGPObjectFactory(cData.getDataStream());
		 * 
		 * message = pgpFact.nextObject(); }
		 * 
		 * if (message instanceof PGPLiteralData) { PGPLiteralData ld = (PGPLiteralData)
		 * message;
		 * 
		 * InputStream unc = ld.getInputStream(); int ch;
		 * 
		 * while ((ch = unc.read()) >= 0) { out.write(ch); } } else if (message
		 * instanceof PGPOnePassSignatureList) { throw new
		 * PGPException("Encrypted message contains a signed message - not literal data."
		 * ); } else { throw new
		 * PGPException("Message is not a simple encrypted file - type unknown."); }
		 * 
